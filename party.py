...
class Party(metaclass=PoolMeta):
    ...
    def write(cls, *args):
        ...
        if identifiers:
            Identifier.reset_aeat_validation(identifiers)

    @classmethod
    def check_aeat(cls, parties):
        Identifier = Pool().get('party.identifier')
        return Identifier.check_aeat([
                identifier for party in parties
...
class Identifier(metaclass=PoolMeta):
    ...
    @classmethod
    def check_aeat(cls, identifiers=None):
        pool = Pool()
        ...
        if identifiers is None:
            identifiers = cls.search([
                    ...
                    ('party.active', '=', True),
                    ['OR',
                        ('type', 'in', ['es_cif', 'es_dni', 'es_nie', 'es_vat']),
                        [('type', '=', 'eu_vat'), ('code', 'like', 'ES%')],
                        ],
                    ['OR',
                        ('aeat_valid', '=', False),
                        ('aeat_validated_at', '=', None),
                        ('aeat_validated_at', '<', datetime.now()
                            - configuration.identifier_aeat_validation_period),
                        ],
                    ], order=[])
        identifiers = [i for i in identifiers if i.active and i.aeat_nif]
        if not identifiers:
            return {'parties': []}
        certificate = configuration.aeat_certificate
        if not certificate:
            ...
            for batch in grouped_slice(identifiers, AEAT_BATCH_SIZE):
                ...
                for identifier, taxpayer, result in zip(
                    ...
                    with without_check_access():
                        cls.write([identifier], {
                            'aeat_valid': valid,
                            'aeat_validated_at': checked_at,
                            })
        ...
        if failed:
            ...
            if len(failed) > 5:
                names += '...'
            Cron.notify(
                'tryton-error', 'party.act_identifier_form', {
                    'pyson_domain': PYSONEncoder().encode(
                        [('id', 'in', [i.id for i in failed])]),
                    },
                'aeat_party_validation.msg_party_identifier_aeat_invalid',
                identifiers=names)
        return default
    ...
    def write(cls, *args):
        ...
        super().write(*args)
        if changed:
            cls.reset_aeat_validation(list(changed))

    @classmethod
    @without_check_access
    def reset_aeat_validation(cls, identifiers):
        # These values are maintained internally, not supplied by the client.
        cls.write(identifiers, {
                'aeat_valid': False,
                'aeat_validated_at': None,
                })
